...
class Error:
    ...
    async def log_error(self) -> int:
        ...
        ret = f"Error_{self.e}: {self.ctx.author} at {y} used {self.ctx.command}. ec_{self.level}: {err[self.level]}.";
        print(ret);
        if dat["verbose"]:
            await self.ctx.send(f"```{ret}```");
        return (0, ret);

# Log

async def log_use(ctx, *args) -> tuple:
    x = datetime.now()
    ret = f"Log_command: {ctx.author} used command: {ctx.command}. Arguments: {args}. Log generated: {x.strftime("%d/%m/%Y %H:%M:%S")}.";
    print(ret);
    if dat["verbose"]:
...
async def eval(ctx, *args) -> None:
    if await log_use(ctx, args):
        if ctx.author.id == bot_owner:
            if args[0] in list(eva.keys()):
                rv = await eva[args[0]](ctx, args);
                if rv == ():
                    await throw_error(ctx, args, 4);
        else:
